...
tray_stop_L:zero tray_stop_R:zero trays_tie:zero
...
ks_braces_present:block ks_latch_room:zero
...
ov_tray_left:zero ov_tray_right:zero ov_keystone:zero ov_front_lip:zero
...
tie_rear:zero tie_screws:zero rear_rib:block bay_spine:block lead_window:zero cord_hole:zero"
fail=0
for t in $TESTS; do
  name=${t%%:*}; kind=${t##*:}
